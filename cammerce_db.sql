...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 07, 2025 at 09:44 AM
...
INSERT INTO `customer` (`customer_id`, `user_id`, `fname`, `lname`, `phone`, `address`) VALUES
(1, 3, '', '', '', '');
...
(3, 'user@gmail.com', '$2y$10$IuXJD9d.SoljTJ/hUJrnX.q9ehz1AFWLGibTO5/DnZKj/oaYLZzLG', 'customer');
...
,`order_date` datetime
,`order_status` enum('pending','shipped','completed','cancelled')
,`payment_method` enum('COD','Online Payment')
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_transaction_details`  AS SELECT `o`.`order_id` AS `order_id`, concat(`c`.`fname`,' ',`c`.`lname`) AS `customer_name`, `u`.`email` AS `customer_email`, `o`.`order_date` AS `order_date`, `o`.`status` AS `order_status`, `o`.`payment_method` AS `payment_method`, `p`.`payment_status` AS `payment_status`, `p`.`amount` AS `total_paid` FROM (((`orders` `o` join `users` `u` on(`o`.`user_id` = `u`.`user_id`)) join `customer` `c` on(`c`.`user_id` = `u`.`user_id`)) left join `payments` `p` on(`o`.`order_id` = `p`.`order_id`)) ;
